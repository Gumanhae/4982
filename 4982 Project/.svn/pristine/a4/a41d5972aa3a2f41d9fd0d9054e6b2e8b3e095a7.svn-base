...
public class ProductListRestController {
	...
	@GetMapping("/productLists/Tag")
	public ResponseEntity<?> findPageByTag(@RequestParam(defaultValue = "1") int pageno,@RequestParam String pname){
		return ResponseEntity.ok(service.findPageByTag(pageno, pname));
	}
}
