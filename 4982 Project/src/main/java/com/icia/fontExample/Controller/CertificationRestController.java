...
public class CertificationRestController {
	...
	public ResponseEntity<?> email_Certification(Principal principal) {
		return ResponseEntity.ok(service.email_Certification(principal.getName()));
	}
	
	@PostMapping(path="/tel_Certification", produces="text/plain;charset=utf-8")
	public ResponseEntity<?> tel_Certification(Principal principal) {
		return ResponseEntity.ok(service.tel_Certification(principal.getName()));
	}
