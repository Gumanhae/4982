...
public class PaymentRestController {
	...
	public ResponseEntity<?> paycomplete(String imp_uid,long amount,Principal principal) 
	...
	@GetMapping("/payment")
	public ResponseEntity<?> paymentByAdmin(@RequestParam(defaultValue = "1")int pageno,
			@RequestParam(defaultValue = "10")int pagesize){
		return ResponseEntity.ok(service.findAllByPage(pageno, pagesize));
	}
	@GetMapping("/payment/user")
	public ResponseEntity<?> paymentByUser(@RequestParam(defaultValue = "1")int pageno,
	...
	@GetMapping("/point")
	public ResponseEntity<?> findPointByUsername(Principal principal,String username){
		String name = "";
		if(username!=null) {
			name = username;
		} else {
		...
		return ResponseEntity.ok(service.findPointByUsername(name));
	}
