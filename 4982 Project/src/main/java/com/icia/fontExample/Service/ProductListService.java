...
public class ProductListService {
	...
	public ProListPage findPageByTag(int pageno, String pname) {
		int count = dao.findCountByTag(pname);
		int startRowNum = ((pageno-1) * pagesize + 1);
		int endRowNum = startRowNum + pagesize -1;
		if(endRowNum >= count)
				endRowNum = count;
		List<Sell> sell = dao.findPageByTag(startRowNum, endRowNum, pname);
		return new ProListPage().builder().pageno(pageno).pagesize(pagesize).totalcount(count).sell(sell).build();
	}
}
