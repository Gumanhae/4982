...
public class MemberService {
	...
	public boolean checkId(String username) {
		return memberDao.checkId(username)==null;
	}
	public boolean checkEmail(String email) {
		return memberDao.checkEmail(email)==null; 
	}
	...
	public String changeSajin(String username,MultipartFile upload) throws IllegalStateException, IOException {
		if (upload != null) {
			if (upload.getContentType().toLowerCase().startsWith("image/")) {
				int lastIndexOfDot = upload.getOriginalFilename().lastIndexOf('.');
				String extension = upload.getOriginalFilename().substring(lastIndexOfDot);
				String imageName = username + extension;
				File file = new File(PROFILE_FOLDER, imageName);
				upload.transferTo(file);
				String fileUrl = PROFILE_URI + imageName;
				memberDao.changeSajin(fileUrl, username);
			}
			return "사진 변경이 완료 되었습니다.";
		}else {
			return "사진 변경에 실패 했습니다.";
		}
	...
	public String findProfilByUsername(String username) {
		return memberDao.findProfilByUsername(username);
	}

	public String telCertification(String tel, String random) {
		return "인증번호가 발송되었습니다.";
	}
	public int withdrawal(String username, String reason, String reason_detail) {
		WithdrawalDao.insert(reason_detail);
		WithdrawalDao.update(reason);
		SalesRightsdao.delete(username);
		authorityDao.delete(username);
		return WithdrawalDao.delete(username);
	}
