...
function printPage(page,params,type) {
	...
	if(type=="all"){
		...
		$(".event").on("click",function(e){
			e.preventDefault();
			findProductBySearch(params.pname,"name",$(this).val());
		})
	}
	else if(type=="tag"){
		$(".event").on("click",function(e){
			e.preventDefault();
			findProductBySearch(params.pname,"tag",$(this).val());
		})
	}
}
...
function findProductBySearch($Pname,$Type,$PageNo){
	var params = {
			pname : $Pname,
			pageno : $PageNo,
		}
	if($Type=="name"){
		...
		$.ajax({
			url : "/fontExample/api/productLists/Tag",
			method : "get",
			data : params,
			success : function(result){
				printList(result);
				printPage(result,params,"tag");
			}
		})
	}
}
...
$(function() {
	...
	loadLarge();
	if(Search!=null){
		findProductBySearch(Search,searchType,1);
		loadSmallByLno(1)
